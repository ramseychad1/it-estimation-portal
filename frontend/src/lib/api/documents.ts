...
/** Upload a new attachment for a specific question on a DRAFT item. */
...
export async function deleteAnswerDocument(attachmentId: number): Promise<void> {
  const token = readCookie(CSRF_COOKIE);
  const headers: Record<string, string> = {};
  if (token) headers[CSRF_HEADER] = token;

  const res = await fetch(`/api/documents/${attachmentId}`, {
    method: "DELETE",
    credentials: "include",
    headers,
  });
