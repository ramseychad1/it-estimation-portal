...
import { useTeamHistoryQuery } from "../../lib/queries/teams";
...
export function TeamHistoryDrawer({ open, teamId, teamName, onClose }: TeamHistoryDrawerProps) {
  const { data, isLoading, isError } = useTeamHistoryQuery(open ? teamId : null);

  return (
    <Drawer
      open={open}
      onClose={onClose}
      title={teamName ? `History — ${teamName}` : "History"}
      subtitle="A record of every change made to this team."
    >
