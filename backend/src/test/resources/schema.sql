...
-- ---- Phase 6a: estimate_requests + per-request snapshots -----------------
-- See V10 migration for design rationale (snapshot-on-submit, audit-FK
-- relaxation, and the sub-feature ↔ product invariant enforced at the
-- service layer rather than as a DB CHECK).
...
CREATE TABLE estimate_request_phase_lines (
    ...
    CONSTRAINT estimate_request_phase_lines_request_fk
...
CREATE TABLE estimate_request_question_answers (
    ...
    CONSTRAINT estimate_request_question_answers_request_fk
        FOREIGN KEY (estimate_request_id) REFERENCES estimate_requests (id) ON DELETE CASCADE,
